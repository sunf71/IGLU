...
IGLUShaderStage::IGLUShaderStage( uint type, const char *inputShader, bool verbose ) : 
	...
	m_verbose(verbose), m_isCompiled(false), m_internalEnables(0), m_internalDisables(0)
{ 
...
{
	// Reset the internal enables/disables.  Really only important on a reload.
	m_internalEnables = 0;
	m_internalDisables = 0;
	...
	while( pch )
	{
			...
			if (pLine) // If yes...
			{
				...
				if (pChar && pChar > pLine) //  Yes, there's a ;, and it's after the :.  Good.
				{
					...
					if ( attribIdx != IGLU_ATTRIB_UNKNOWN )
					{
						...
						iglu::Warning( "GLSL shader uses obsolete attribute IGLU semantic (e.g.,\n"
							           "    something like:  'in vec3 myVertex : IGLU_VERTEX;'\n"
									   "       rather than:  'layout(location = IGLU_VERTEX) in vec3 myVertex;' )\n\n"
									   "This obsolete CG-style format will eventually go away in IGLU.  Please update!\n",
		...
		else if( !strcmp( firstToken, "layout" ) )
		{
			...
			if (pChar != NULL) // If there was one
			{
				...
				case IGLU_MRT_COLOR0:
					bufPtr[0] = '0';
					break;
				case IGLU_ATTRIB_NORMAL:
				case IGLU_MRT_COLOR1:
					bufPtr[0] = '1';
					break;
				case IGLU_ATTRIB_TEXCOORD:
				case IGLU_MRT_COLOR2:
					bufPtr[0] = '2';
					break;
				case IGLU_ATTRIB_MATL_ID:
				case IGLU_MRT_COLOR3:
					bufPtr[0] = '3';
					break;
				case IGLU_ATTRIB_OBJECT_ID:
				case IGLU_MRT_COLOR4:
					bufPtr[0] = '4';
					break;
				case IGLU_MRT_COLOR5:
					bufPtr[0] = '5';
					break;
				case IGLU_MRT_COLOR6:
					bufPtr[0] = '6';
					break;
				case IGLU_MRT_COLOR7:
					bufPtr[0] = '7';
					break;
				}
		...
		else if( !strncmp( line, "#pragma", 7 ) )
		{
			// Check if this is an IGLU pragma
			char *pChar = strstr( line, "IGLU_" );
			if (pChar != NULL) // If there was one
			{
				char token[LINE_SIZE], enable[LINE_SIZE];
				pChar = StripLeadingTokenToBuffer( pChar, token );  // find IGLU_ENABLE or IGLU_DISABLE
				pChar = StripLeadingTokenToBuffer( pChar, enable ); // find setting to ENABLE/DISABLE
				MakeLower( token );
				MakeLower( enable );
				
				uint flag = GetShaderStateFlag( enable );
				if ( !strcmp( token, "iglu_enable" ) )
					m_internalEnables |= flag;
				else if ( !strcmp ( token, "iglu_disable" ) )
					m_internalDisables |= flag;

				// Here: could delete the line so GLSL doesn't process it, but since it shouldn't
				//       understand the pragma, the spec says it should ignore it.
			}
		}
...
uint IGLUShaderStage::GetSemanticType( char *type )
{
	...
	if (!strcmp( type, "iglu_color8" ) || !strcmp( type, "iglu_mrt8" ))
		return IGLU_MRT_COLOR8;
	if (!strcmp( type, "iglu_color9" ) || !strcmp( type, "iglu_mrt9" ))
		return IGLU_MRT_COLOR9;
	if (!strcmp( type, "iglu_color10" ) || !strcmp( type, "iglu_mrt10" ))
		return IGLU_MRT_COLOR10;
	if (!strcmp( type, "iglu_color11" ) || !strcmp( type, "iglu_mrt11" ))
		return IGLU_MRT_COLOR11;
	if (!strcmp( type, "iglu_color12" ) || !strcmp( type, "iglu_mrt12" ))
		return IGLU_MRT_COLOR12;
	if (!strcmp( type, "iglu_color13" ) || !strcmp( type, "iglu_mrt13" ))
		return IGLU_MRT_COLOR13;
	if (!strcmp( type, "iglu_color14" ) || !strcmp( type, "iglu_mrt14" ))
		return IGLU_MRT_COLOR14;
	if (!strcmp( type, "iglu_color15" ) || !strcmp( type, "iglu_mrt15" ))
		return IGLU_MRT_COLOR15;
	return IGLU_ATTRIB_UNKNOWN;
}


uint IGLUShaderStage::GetShaderStateFlag( char *type )
{
	if ( !strcmp( type, "blend" ) || !strcmp( type, "blend_test" ) || !strcmp( type, "blending" ) )
		return IGLU_GLSL_BLEND;
	if (!strcmp( type, "depth_test" ) || !strcmp( type, "zbuffer" ) || !strcmp( type, "depth" ) )
		return IGLU_GLSL_DEPTH_TEST;
	if (!strcmp( type, "stencil_test" ) || !strcmp( type, "stenciling" ) || !strcmp( type, "stencil" ) )
		return IGLU_GLSL_STENCIL_TEST;
	if (!strcmp( type, "scissor_test" ) || !strcmp( type, "scissoring" ) || !strcmp( type, "scissor" ) )
		return IGLU_GLSL_SCISSOR_TEST;
	if ( !strcmp(type, "cull_face" ) || !strcmp( type, "culling" ) || !strcmp(type, "cull") )
		return IGLU_GLSL_CULL_FACE;
	if (!strcmp( type, "point_size" ) || !strcmp( type, "pointsize" ) || !strcmp( type, "vary_point_size" ) || !strcmp( type, "varying_point_size" ) )
		return IGLU_GLSL_VARY_POINT_SIZE;
	if (!strcmp( type, "rasterize_discard" ) || !strcmp( type, "rasterizer_discard" ) || !strcmp( type, "raster_discard" ) )
		return IGLU_GLSL_RASTERIZE_DISCARD;
	if (!strcmp( type, "line_smooth" ) || !strcmp( type, "smooth_lines" ))
		return IGLU_GLSL_LINE_SMOOTH;
	if (!strcmp( type, "multisample" ) || !strcmp( type, "multisampling" ))
		return IGLU_GLSL_MULTISAMPLE;
	if (!strcmp( type, "logic_op" ))
		return IGLU_GLSL_LOGIC_OP;
	if (!strcmp( type, "color_logic_op" ))
		return IGLU_GLSL_COLOR_LOGIC_OP;
	if (!strcmp( type, "sample_shading" ) || !strcmp( type, "multisample_shading" ))
		return IGLU_GLSL_SAMPLE_SHADING;
	if (!strcmp( type, "sample_mask" ) || !strcmp( type, "multisample_mask" ))
		return IGLU_GLSL_SAMPLE_MASK;
	
	return IGLU_GLSL_DEFAULT_STATE;
}
