...
{	
	// Resize all depth buffer(s).  Should only be one, but let's be safe.
	for(uint i = 0; i < m_depthBufs.Size(); i++)
	{
		if(m_depthBufs[i]->Is2DTexture())
			((IGLURenderTexture2D*)m_depthBufs[i])->Resize(width, height);
	}
	
	// Resize all stencil buffer(s).  Should only be one, but let's be safe.
	for(uint i = 0; i < m_stencilBufs.Size(); i++)
	{
		if(m_stencilBufs[i]->Is2DTexture())
			((IGLURenderTexture2D*)m_stencilBufs[i])->Resize(width, height);
	}
