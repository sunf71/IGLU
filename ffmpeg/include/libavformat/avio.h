...
typedef struct URLContext {
    ...
    int is_connected;
    void *control_opaque; /**< an URLContext related controler handle */
} URLContext;
...
typedef int URLInterruptCB2(void *opaque);
...
/* The URLContext related callback, it is binded to a specific URLContext
 * while url_set_interrupt_cb() is a global counterpart
 * we just try to refine the FFmpeg's original design
 */
void url_set_interrupt_cb2(URLInterruptCB2 *interrupt_cb);
...
extern URLInterruptCB2 *url_interrupt_cb2;
...
/*
 * Share memory buffer used for "mem://address" input&output protocol
 * by IPC communication
 */
typedef struct {
    /* MAGIC code: stub for 'mem://address' , the MAX address is 128bit */
    char protocol[6+16+1];
    unsigned char *sm_buffer;
